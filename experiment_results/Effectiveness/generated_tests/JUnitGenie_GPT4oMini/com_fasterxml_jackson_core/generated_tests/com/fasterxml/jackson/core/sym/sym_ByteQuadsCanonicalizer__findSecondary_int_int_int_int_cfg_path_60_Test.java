...
public class sym_ByteQuadsCanonicalizer__findSecondary_int_int_int_int_cfg_path_60_Test {
    ...
    public void testFindSecondary() {
        try {
            ...
            setField(instance, "_hashArea", new int[]{1, 2, 3, 3, 1, 2, 3, 3, 0, 0, 0, 0});
            setField(instance, "_names", new String[]{"name1", "name2"});
            setField(instance, "_tertiaryShift", 2);
            setField(instance, "_tertiaryStart", 0);
            setField(instance, "_spilloverEnd", 28); // Ensure _hashSize < _spilloverEnd / 7

            // Test case where the values match
            String result = invokeFindSecondary(instance, 1, 2, 3, 3);
            assertEquals("name1", result);

            // Test case where the values do not match
            result = invokeFindSecondary(instance, 4, 5, 6, 3);
            assertNull(result);
    ...
    private String invokeFindSecondary(ByteQuadsCanonicalizer instance, int origOffset, int q1, int q2, int q3) {
        try {
