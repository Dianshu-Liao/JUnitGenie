...
public class util_JsonParserSequence_createFlattened_boolean_JsonParser_JsonParser_cfg_path_6_Test {

    private static class ConcreteJsonParser extends JsonParser {
        ...
        @Override
        public byte[] getBinaryValue(Base64Variant variant) { return new byte[0]; }
        @Override
    ...
    @Test(timeout = 4000)
    public void testCreateFlattenedWithJsonParsers() {
        JsonParser firstParser = new ConcreteJsonParser();
        JsonParser secondParser = new ConcreteJsonParser();
        boolean checkForExistingToken = true;

        JsonParserSequence result = null;
        try {
            result = JsonParserSequence.createFlattened(checkForExistingToken, firstParser, secondParser);
        } catch (Exception e) {
