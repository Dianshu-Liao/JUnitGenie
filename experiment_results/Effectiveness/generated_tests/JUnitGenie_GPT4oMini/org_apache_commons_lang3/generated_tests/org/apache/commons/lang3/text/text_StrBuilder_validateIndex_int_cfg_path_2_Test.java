...
public class text_StrBuilder_validateIndex_int_cfg_path_2_Test {
    ...
    public void testValidateIndexThrowsExceptionForIndexGreaterThanSize() {
        StrBuilder strBuilder = new StrBuilder();
        strBuilder.setLength(5); // Set size to 5
        int outOfBoundsIndex = 6;

        try {
            // Accessing the protected method using reflection
            Method method = StrBuilder.class.getDeclaredMethod("validateIndex", int.class);
            method.setAccessible(true);
            method.invoke(strBuilder, outOfBoundsIndex);
