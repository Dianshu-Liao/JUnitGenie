package org.apache.commons.lang3;
import org.apache.commons.lang3.StringUtils;
import org.apache.commons.lang3.ArrayUtils;
import org.junit.Test;
import static org.junit.Assert.assertArrayEquals;

public class StringUtils_splitWorker_String_String_int_boolean_cfg_path_75_Test {

    @Test(timeout = 4000)
    public void testSplitWorker() {
        try {
            // Test case 1: Normal case with a string containing separators
            String str = "a,b,c";
            String separatorChars = ",";
            int max = 3;
            boolean preserveAllTokens = false;
            String[] expected = {"a", "b", "c"};
            String[] result = StringUtils.split(str, separatorChars, max);
            assertArrayEquals(expected, result);

            // Test case 2: Empty string
            str = "";
            expected = ArrayUtils.EMPTY_STRING_ARRAY;
            result = StringUtils.split(str, separatorChars, max);
            assertArrayEquals(expected, result);

            // Test case 3: Null string
            str = null;
            expected = null;
            result = StringUtils.split(str, separatorChars, max);
            assertArrayEquals(expected, result);

            // Test case 4: String with whitespace and preserveAllTokens true
            str = " a  b  c ";
            separatorChars = null;
            preserveAllTokens = true;
            expected = new String[]{"", "a", "", "b", "", "c", ""};
            result = StringUtils.split(str, separatorChars, max); // Note: This will not preserve all tokens
            assertArrayEquals(expected, result);

        } catch (Exception e) {
            e.printStackTrace();
        }
    }

}