...
public class DateTimeZone_forOffsetHoursMinutes_int_int_cfg_path_14_Test {

    @Test(timeout = 4000)
    public void testForOffsetHoursMinutes_ValidInput() {
        // Test with valid input that leads to a positive offset
        DateTimeZone zone = DateTimeZone.forOffsetHoursMinutes(1, 30);
        assertNotNull(zone);
    }
    
    @Test(timeout = 4000)
    public void testForOffsetHoursMinutes_InvalidHours() {
        try {
            DateTimeZone.forOffsetHoursMinutes(24, 0);
            fail("Expected IllegalArgumentException for hours offset out of range");
    ...
    public void testForOffsetHoursMinutes_InvalidMinutes() {
        try {
            ...
            fail("Expected IllegalArgumentException for minutes offset out of range");
        } catch (IllegalArgumentException e) {
