...
def get_code_refinement_prompts_for_our_approach(our_approach_result_after_formatting_path, syntax_error_messages_dir):
    ...
    our_approach_result['second_round_syntax_error_message'] = ''
    our_approach_result['second_refinement_prompt'] = ''


    for index, row in tqdm.tqdm(our_approach_result.iterrows(), total=our_approach_result.shape[0]):
        test_code_after_formatting = row['test_code_after_first_refinement_after_formatting']
        ...
        #     corresponding_row = all_syntax_error_messages[all_syntax_error_messages['file_name'] == saved_path]
            ...
            if pd.isna(error_message):
                our_approach_result.loc[index, 'second_round_syntax_error_message'] = 'no syntax error thus no refinement needed'
                our_approach_result.loc[index, 'second_refinement_prompt'] = 'no syntax error thus no refinement needed'
                continue

            second_refinement_prompt = construct_code_refinement_prompt(error_message, test_code_after_formatting)
            our_approach_result.loc[index, 'second_round_syntax_error_message'] = error_message.replace('\n', '\\n')
            our_approach_result.loc[index, 'second_refinement_prompt'] = str(second_refinement_prompt)
        else:
            our_approach_result.loc[index, 'second_round_syntax_error_message'] = 'no syntax error thus no refinement needed'
            our_approach_result.loc[index, 'second_refinement_prompt'] = 'no syntax error thus no refinement needed'
...
def process_refinement_row(index, row, api_key):
    ...
    second_refinement_prompt = eval(row['second_refinement_prompt'])
    ...
    while True:
        try:
            test_code_after_second_refinement = LLM_Utils.trigger_GPT_API_basedon_http_request(
                second_refinement_prompt, model=Config.foundation_model_gpt4o_mini, openai_key=api_key
            )
            ...
            test_code_after_second_refinement_after_formatting = get_runnable_code_from_test_code(
                test_code_after_second_refinement, package_import_name, class_name
            )

            return index, test_code_after_second_refinement, test_code_after_second_refinement_after_formatting
...
def get_test_code_after_refinement(our_approach_result_after_first_round_refinement_path, our_approach_result_after_second_round_refinement_path, syntax_error_messages_dir):
    """Parallel processing for refining test code."""
    our_approach_result = get_code_refinement_prompts_for_our_approach(
        our_approach_result_after_first_round_refinement_path, syntax_error_messages_dir
    )
    core_project_rows = our_approach_result[our_approach_result['project_dir'].str.contains('com_fasterxml_jackson_core')]
    no_syntax_error_rows = core_project_rows[core_project_rows['second_round_syntax_error_message'] == 'no syntax error thus no refinement needed']
    syntax_error_rows = core_project_rows[core_project_rows['second_round_syntax_error_message'] != 'no syntax error thus no refinement needed']

    our_approach_result['test_code_after_second_refinement'] = ''
    our_approach_result['test_code_after_second_refinement_after_formatting'] = ''
    ...
    for index, row in tqdm.tqdm(our_approach_result.iterrows(), total=our_approach_result.shape[0]):
        ...
        if row['second_refinement_prompt'] == 'no syntax error thus no refinement needed':
            test_code_after_first_refinement = row['test_code_after_first_refinement_after_formatting']
            our_approach_result.at[index, 'test_code_after_second_refinement'] = test_code_after_first_refinement
            our_approach_result.at[index, 'test_code_after_second_refinement_after_formatting'] = test_code_after_first_refinement
    ...
    with ThreadPoolExecutor(max_workers=len(api_key_list)) as executor:
        ...
        for future in tqdm.tqdm(as_completed(futures), total=len(futures)):
            ...
            our_approach_result.at[index, 'test_code_after_second_refinement'] = test_code_after_second_refinement
            our_approach_result.at[index, 'test_code_after_second_refinement_after_formatting'] = test_code_after_second_refinement_after_formatting

            current_num += 1
            if current_num % batch_size == 0:
                our_approach_result.to_csv(our_approach_result_after_second_round_refinement_path, index=False)
                print(f"[INFO] Saved {current_num} rows to {our_approach_result_after_second_round_refinement_path}")


    # Final save
    our_approach_result.to_csv(our_approach_result_after_second_round_refinement_path, index=False)
    print(f"[INFO] Saved all rows to {our_approach_result_after_second_round_refinement_path}")
...
def write_all_formatted_code_to_test_dir(our_approach_result_after_refinement_path):
    ...
    for index, row in tqdm.tqdm(our_approach_result_after_refinement.iterrows(),
        ...
        saved_path = row['saved_path']
        test_code_after_refinement_after_formatting = row['test_code_after_second_refinement_after_formatting']
...
if __name__ == '__main__':
    ...
    get_test_code_after_refinement(our_approach_result_after_first_round_refinement_path, our_approach_result_after_second_round_refinement_path, syntax_error_messages_dir)
    ...
    write_all_formatted_code_to_test_dir(our_approach_result_after_second_round_refinement_path)
